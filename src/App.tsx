...
export default function App() {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    setNewStudent({ fullName: '', grade: '', diagnosis: '', resolution: '', accommodationType: 'Adecuación de Acceso', photoUrl: '' });
    ...
    try {
      channel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'students' }, () => {
          // Debounce or delay slightly to ensure DB consistency before refetching
          // Use background fetch to avoid UI flicker
          setTimeout(() => fetchStudents(true), 500);
        })
        .subscribe();
    } catch (err) {
      console.warn("Realtime subscription subscription error:", err);
    }
                    ...
                    >
                      <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center shrink-0 bg-slate-50/50">
                        <h3 className="text-xl md:text-2xl font-black text-slate-800 font-display">
                          {editingStudent ? "Editar Registro" : "Nuevo Registro Estudiantil"}
                        </h3>
                        <button 
                          onClick={() => {
                            setIsAddingStudent(false);
                      ...
                      <form onSubmit={handleAddStudent} className="p-6 md:p-8 space-y-6 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 pt-6 shrink-0">
                          ...
                          >
                            {editingStudent ? "Actualizar Registro" : "Guardar Registro"}
                          </button>
                        </div>
                      </form>
                    </motion.div>
